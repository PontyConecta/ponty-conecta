...
export default function AdminHeader({ currentPageName }) {
  return (
    <nav className="flex items-center gap-0.5 overflow-x-auto scrollbar-hide rounded-lg p-1 bg-muted">
      {adminNav.map(item => {
        ...
        return (
          <Link
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-md whitespace-nowrap transition-all ${
              isActive ? 'shadow-sm bg-card text-primary' : 'text-muted-foreground hover:text-foreground'
            }`}
          >
