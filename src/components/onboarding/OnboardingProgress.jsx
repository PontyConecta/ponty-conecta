      ...
      {steps.map((s, index) => (
        <React.Fragment key={s.number}>
          ...
          >
            ...
            <span className={`hidden sm:block text-xs sm:text-sm whitespace-nowrap ${currentStep >= s.number ? 'font-medium' : ''} ${s.number < currentStep && onStepClick ? 'group-hover:text-[#9038fa]' : ''}`}
              style={{ color: currentStep >= s.number ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
              {s.title}
            </span>
          </div>
          {index < steps.length - 1 && (
            <div className="w-4 sm:w-8 h-0.5 flex-shrink-0" style={{ backgroundColor: currentStep > s.number ? '#9038fa' : 'var(--border-color)' }} />
          )}
        </React.Fragment>
      ))}
    </div>
